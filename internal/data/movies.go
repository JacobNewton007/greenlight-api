...
import (
	...
	"github.com/JacobNewton007/greenlight/internal/validator"
	"github.com/lib/pq"
)
...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(movie.Year <= int32(time.Now().Year()), "year", "must not be in the futur")
	...
	v.Check(validator.Unique(movie.Genres), "genres", "must not be duplicate values")
...
func (m MovieModel) Insert(movie *Movie) error {
	...
	// the movie struct. Declaring the slice immediately next our SQL query helps to
...
// Add a placeholder method for deleting a specific record in the movie table
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	// Use QueryContext() to execute the query. This return a sql.Rows resultset
	...
	// Initialize and empty slice to hold the movie data.
	totalRecords := 0
	movies := []*Movie{}

	// Use the rows.Next to iterate through the rows in the resultset.
	for rows.Next() {
		// Initialize an empty Movie struct to hold the movie data for an Individual movie.
		var movie Movie
	...
	// When the row.Next() loop has finished, call rows.Err() to retrieve any error
	// that was encountered during the iteration.
	if err = rows.Err(); err != nil {
		return nil, Metadata{}, err
	}
	// Generate a Metadata struct. passing in the total record count and the pagination
